...
// sets up listener for user dragging or zooming map - state can't be accessed inside google map objects / event listeners directly, so this is a workaround
...
function ResultsMap(props) {
  ...
  const [mapDragged, setMapDragged] = useState();
...
`;
  ...
  const renderMap = () => {
    ...
    map.addListener("click", (event) => {
      // getter/setter triggers function outside event listener (where current state cannot be accessed)
  ...
  mapMoveListener.registerListener(function (val) {
    ...
    if (val) {
      ...
      if (mapDragged || mapZoomed || props.searchLocationUpdate) {
        console.log("map drag or zoom");
        const newMapParameters = {
          bounds: map.getBounds(),
          center: map.getCenter(),
          zoom: map.getZoom(),
